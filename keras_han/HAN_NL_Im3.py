...
from keras.layers import (
    ...
    Embedding, Bidirectional, Lambda, Concatenate
)
...
class HAN(Model):
    def __init__(
        ...
        in_tensor, in_pseduoLabels, in_matrix,  prediction, out_tensor = self._build_network()

        super(HAN, self).__init__(
            inputs=[in_tensor, in_pseduoLabels, in_matrix], outputs= [prediction, out_tensor], name=name
        )
    ...
    def _build_network(self):
        ...
        # pseduo label通过网络产生:19*
        in_pseduoLabels = Input(shape=(self.output_size,))
        in_matrix = Input(shape=(self.output_size,))
        ...
        ##定义损失矩阵，对网络输出进行调整############
        #cost_out = keras.layers.Multiply()([prediction, in_matrix])
        #############
        # dense1 = Dense(self.output_size, activation='softmax', name='dense1')(in_pseduoLabels)
        # weight_tensor = keras.layers.Multiply()([prediction, in_pseduoLabels])
        # cost_tensor = keras.layers.Multiply()([weight_tensor, in_matrix])
        #out_tensor = 1./(1 + np.exp(-im_weight_tensor))
        # dense2 = Dense(self.output_size, activation='softmax', name='dense2')(dense1)
        # merge = Concatenate()([prediction, dense2])
        #out_tensor = Dense(self.output_size, activation='softmax', name='output')(merge)
        merge = Concatenate()([prediction, in_pseduoLabels])
        merge_out = Dense(self.output_size, activation='softmax', name='output')(merge)
        out_tensor = keras.layers.Multiply()([merge_out, in_matrix])
        return in_tensor, in_pseduoLabels, in_matrix,  prediction, out_tensor
